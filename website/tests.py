...
class ProductDetailViewTest(TestCase):
    ...
    def test_product_detail_view_shows_correct_product(self):
        response = self.client.get(
            reverse('website:single_product', args=([self.product.pk])))
...
class ProductTypeViewTest(TestCase):
    """
    Purpose: Verify that when a specific product type is chosen, 
    only products in that type are displayed.
    ...
    def setUp(self):
        ...
        self.product_type1 = ProductType.objects.create(
            product_type_name="TestType1")
        self.product_type2 = ProductType.objects.create(
            product_type_name="TestType2")
    ...
    def test_product_type_products_view(self):
        response = self.client.get(
            reverse('website:get_product_types', args=([self.product_type1.pk])))  
        self.assertQuerysetEqual(
            response.context['products_of_type'],
            ["<Product: Magic Hat>", "<Product: Magic Wand>"], ordered=False)


class PaymentTypesViewTest(TestCase):
    """
    Purpose: Verifies that the Payment Types view for a customer 
    has all of the payment types in the request context
    Author: Dara Thomas
    Args: HTTP request
    Returns: Pass/Fail based on successful/unsuccessful assertion
    """
    ...
    def test_payment_type_view_shows_payment_types(self):
        ...
        self.assertQuerysetEqual
        (response.context["user_payment_types"],
            ["<PaymentType: AMEX>",
...
class ProductsInCartViewTest(TestCase):
    """
    Purpose: Verify that when products are added to an order, 
    that the Order Summary view has those products in the response context
    Author: Dara Thomas
    Args: (integer) product pk
    Returns: Pass/Fail based on successful/unsuccessful assertion
    """
    ...
    def test_products_show_in_cart(self):
        ...
        self.product_type = ProductType.objects.create(
            product_type_name="TestProdType")
        ...
        self.assertQuerysetEqual
        (response.context["products_in_cart"],
            ["<ProductOrder: emoji stickers>",
                "<ProductOrder: Keys>", "<ProductOrder: Magic Wand>"])


class OrderHistoryViewTest(TestCase):
    """
    Purpose: to test that the order history is showing orders 
    that correspond with the authenticated user
